...
def get_data():
    ...
    for i,(model_name,model) in enumerate(good_models):
        ...
        closest_mass = np.array([closest_mass]*len(closest_r)).reshape((len(closest_r)))
        
        metadata.append((model_name,closest_age,distance))
        data.append((closest_r,closest_disp,weights,closest_mass))

    X = [np.array((x[0],x[3])) for x in data]
    #X = [x[np.newaxis,:] for x in X]
    Y = [np.array(x[1]) for x in data]
    weights = [np.array(x[2]) for x in data]

    print(len(X),len(Y),len(weights),X[0].shape,Y[0].shape,weights[0].shape)
...
def run_sr_analysis(X, y, weights, run_logger=None, run_visualiser=None):
    ...
    expression, logs = physo.ClassSR(X, Y, weights,
                                X_units=[[1,0,0],[0,0,1]], X_names=["r","m"], 
                                y_units=[1,-1,0], y_name='sigma', 
                                class_free_consts_units=[[1,0,0], [1,-1,0], [0,1,0],[0,0,1]],
                                class_free_consts_names=['r_s', 'v_c', 'T','M'],
                                fixed_consts_units=[[3,-2,-1]], 
                                fixed_consts=[6.67430*1e-11],
                                spe_free_consts_names = [ "k0"      , "tau"      , "d"],
                                spe_free_consts_units = [ [0, 0, 0] , [0, 1, 0] , [1, 0, 0] ],
                                op_names=['mul', 'add', 'sub','div', 'inv', 'n2', 'sqrt', 'neg', 'exp', 'log', 'n3'],
                                run_config = config,
                                epochs=250,
                                parallel_mode=True,
                                n_cpus=n_cpus)
